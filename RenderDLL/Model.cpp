...
Model::Model(GLchar* path)
{
	__loadModel(path);
	m_InstanceNumber = 1;
	m_IsInstanceRender = false;
}

void Model::__loadModel(std::string path)
{
...
void Model::__processNode(aiNode* node, const aiScene* scene)
{
	...
	for (GLuint i = 0; i < node->mNumMeshes; i++)
	{
		...
		m_MeshSet.push_back(__processMesh(mesh, scene));
	}
...
Mesh Model::__processMesh(aiMesh* mesh, const aiScene* scene) {
	// Data to fill
...
std::vector<STexture> Model::__loadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName)
{
	std::vector<STexture> textures;
	for (GLuint i = 0; i < mat->GetTextureCount(type); i++)
	{
		...
		{   // If texture hasn't been loaded already, load it
			STexture texture;
			texture.id = TextureFromFile(str.data, m_Directory);
...
void Model::Draw(GLuint shaderID)
{
	for (GLuint i = 0; i < m_MeshSet.size(); i++)
		m_MeshSet[i].Draw(shaderID, m_InstanceNumber, m_IsInstanceRender);
}